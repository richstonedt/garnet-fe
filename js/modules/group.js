...
var vm = new Vue({
    ...
    methods: {
        ...
        update: function (groupId) {
            ...
            /* Promise.all([vm.getTenantList(), vm.getAppList()]).then(function(values){
                vm.initTreesToUpdate(groupId);
            }); */
        ...
        reload: function (backFirst) {
            ...
            $("#jqGrid").jqGrid('setGridParam', {
                postData: {
                    ...
                    applicationId: vm.appSearchList.searchApp,
        ...
        getGroupInfo: function (groupId) {
            $.get(baseURL + "groups/" + groupId, function (response) {
                /* applicationId: 5586992595080
                createdTime: 1558704270668
                id: 5587042706648
                modifiedTime: 1558704492480
                name: "Demo Foo 组"
                status: 1
                tenantId: 5586990368120
                updatedByUserName: "admin" */
                vm.group.groupId = response.data.group.id;//groupId
                ...
                // console.log('>>> set vm.appList.selectedApp=',vm.appList.selectedApp);
                // console.log('>>> set vm.tenantList.selectedTenant=',vm.tenantList.selectedTenant);
                vm.inited = true;

                // vm.getAppList() 依赖 vm.tenantList.selectedTenant
                ...
                // 加载用户树 Promise
                const userTreePromise = new Promise(resolve => {
                    $.get(baseURL + "users/byparams?tenantId=" + selectedTenant + "&applicationId=" + selectedApp, function (res) {
                        userTree = $.fn.zTree.init($("#userTree"), userTreeSetting, res);
                        userTree.expandAll(true);
                        resolve('user tree is ready!');
                    });
                });

                // 加载角色树 Promise
                const roleTreePromise = new Promise(resolve => {
                    $.get(baseURL + "roles/byparams?tenantId=" + selectedTenant + "&applicationId=" + selectedApp, function (res) {
                        roleTree = $.fn.zTree.init($("#roleTree"), roleTreeSetting, res);
                        roleTree.expandAll(true);
                        resolve('role tree is ready!');
                    });
                });

                Promise.all([userTreePromise, roleTreePromise]).then(values => {
                    // 勾选已有用户
        ...
        selectTenant: function () {
            ...
            // console.log('in selectTenant(), call vm.reloadUserTree(), vm.reloadRoleTree()');
            vm.reloadUserTree();
            vm.reloadRoleTree();
        ...
        selectApp: function () {
            ...
            vm.group.tenantId = vm.tenantList.selectedTenant;
            vm.group.applicationId = vm.appList.selectedApp;
            // console.log('in selectApp(), call vm.reloadUserTree(), vm.reloadRoleTree()');
            vm.reloadUserTree();
            vm.reloadRoleTree();
        ...
        reloadUserTree : function() {
            // console.log('in reloadUserTree(), vm.tenantList.selectedTenant=', vm.tenantList.selectedTenant);
            // $.get(baseURL + "users/tenantId/" + vm.tenantList.selectedTenant, function (response) {
            //vm.typeList.selectedType 1租户 2应用 3租户+应用
            if(vm.typeList.selectedType==1 && !vm.tenantList.selectedTenant)return;
            if(vm.typeList.selectedType==2 && !vm.appList.selectedApp)return;
            if(vm.typeList.selectedType==3 && (!vm.tenantList.selectedTenant || !vm.appList.selectedApp))return;
        ...
        reloadUserTreeByApp : function() {
            // console.log('in reloadUserTreeByApp(), vm.appList.selectedApp=', vm.appList.selectedApp);
        ...
        reloadRoleTree : function() {
            // console.log('in reloadRoleTree(), vm.tenantList.selectedTenant=', vm.tenantList.selectedTenant);
        ...
        reloadRoleTreeByApp : function() {
            // console.log('in reloadRoleTreeByApp(), vm.appList.selectedApp=', vm.appList.selectedApp);
        ...
        getGroupsByTeantIdAndAppId: function () {
            vm.searchApp = vm.appSearchList.searchApp;
